...
export function PublicLeagueViewPage() {
	...
	const tournaments = view?.tournaments ?? [];
			...
			<main className="mx-auto grid max-w-6xl gap-6 px-4 py-6">
				...
				<section className="grid gap-3">
					...
					{tournaments.map((tournament) => (
						<article key={tournament.id} className="rounded-lg border border-slate-200 bg-white p-4">
							<div className="flex flex-wrap items-start justify-between gap-3">
								<div>
									<h3 className="font-bold text-ink">{tournament.name}</h3>
									<p className="mt-1 text-sm text-slate-600">{tournament.gameType === 'Pool' ? 'Pool knockout' : 'Darts highest score'} · {tournament.participantCount} players</p>
								</div>
								{tournament.winnerName ? <span className="rounded bg-emerald-100 px-2 py-1 text-xs font-semibold text-emerald-800">Winner {tournament.winnerName}</span> : <span className="rounded bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-700">{tournament.status}</span>}
							</div>
						</article>
					))}
					{tournaments.length === 0 ? <div className="rounded-lg border border-dashed border-slate-300 bg-white p-8 text-center text-sm text-slate-600">No tournaments yet.</div> : null}
				</section>
			</main>
		</div>
	);
