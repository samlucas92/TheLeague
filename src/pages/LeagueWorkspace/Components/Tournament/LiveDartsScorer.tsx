...
export function LiveDartsScorer({ leagueId, tournament, match, canManage, playerOne, playerTwo, onChanged }: { leagueId: string; tournament: Tournament; match: TournamentMatch; canManage: boolean; playerOne: string; playerTwo: string; onChanged: () => Promise<void> }) {
	...
	return (
		<div className="grid gap-4 border-t border-slate-100 pt-4">
			<div className="grid grid-cols-[1fr_auto_1fr] items-stretch gap-2 sm:gap-3 lg:gap-4">
				...
				<div className="grid min-w-16 place-items-center rounded-lg border border-slate-200 px-2 py-2 text-center sm:min-w-24 sm:px-4">
					<div>
						<p className="text-[10px] font-bold uppercase text-slate-500 sm:text-xs">Legs</p>
						<p className="text-xl font-bold text-ink sm:text-3xl">{playerOneLegs} - {playerTwoLegs}</p>
						<p className="text-[10px] font-semibold text-slate-500 sm:text-xs">First to {targetLegs}</p>
					</div>
				</div>
